...
const compareArraysUnordered = (
  ...
  for (let i = 0; i < left.length; i++) {
    ...
    if (matchIndex === -1) {
      ...
      entries.push(createUnchangedEntry([...path, String(i)], leftItem, leftItem));
    }
  }

  // Remaining items in rightCopy are added
  for (const rightItem of rightCopy) {
    entries.push(createAddedEntry([...path, String(right.indexOf(rightItem))], rightItem));
  }
...
const areValuesEqual = (
  ...
  if (typeof left === 'number' && typeof right === 'number' && settings.floatTolerance !== undefined) {
 ...
 */
const createAddedEntry = (path: readonly string[], value: JsonValue): DiffEntry => ({
  type: 'added',
  path,
  rightValue: value,
});
 ...
 */
const createRemovedEntry = (path: readonly string[], value: JsonValue): DiffEntry => ({
  type: 'removed',
  path,
  leftValue: value,
});
