import type { ExportPort, ExportError, ExportOptions } from '../../application/ports/ExportPort';
...
export class ExportAdapter implements ExportPort {
  async exportToFile(
    ...
    const filename = options.filename ?? `export-${Date.now()}.${this.getExtension(options.format)}`;
  ...
  async copyToClipboard(content: string): Promise<Result<void, ExportError>> {
    try {
      ...
      const message = e instanceof Error ? e.message : 'Failed to copy to clipboard';
